...
- 2026-08-25: **Resolved** — deleted `kubernetes-platform/argocd-helm/` and the `helm/core-services/*` + `helm/workers/*` charts it pointed at. They were confirmed orphaned (nothing synced them; the real `root-app.yaml` only ever watched `kubernetes-platform/argocd/apps`) and a repo-wide grep for `argocd-helm`/`helm/core-services`/`helm/workers` turned up no remaining references. **Kustomize is now the sole deployment mechanism** — every service and platform component is sourced from `kustomize/overlays/dev/<service>` (or plain manifests for `keda`/`network-policies`/`observability`) via the ArgoCD Applications under `kubernetes-platform/argocd/apps/`.
